    ...
    def select(self, table_name, condition):
        ...
        if os.path.exists(file_path):
            ...
            try:
                ...
                pass
            cordinats = self.garden[key].search(value)
            with open(file_path, 'r')as file:
            ...
            if data_to_delete:
                ...
                for obj in data_to_delete:
                    ...
                    if self.delete_counter >= FLUSH_THRESHOLD:
                        remaining_data = list(filter(lambda obj: obj.get('id') not in self.delete_markers[table_name],
                                                     all_data[table_name]))
